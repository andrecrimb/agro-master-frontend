...
const EditUserDialog: React.FC = () => {
  ...
  const customerId = +history.location.pathname.replace(`${routes.customers}/`, '')
  ...
  const onClose = () => history.push(routes.customers)
  ...
  return (
    <Dialog open fullWidth maxWidth="sm" onClose={onClose} aria-labelledby="dialog-title">
      <DialogTitle id="dialog-title">
        {t('edit_customer')} |{' '}
        <span style={{ color: muiTheme.palette.primary.main }}>{customerSelected?.firstName}</span>
      </DialogTitle>
      {customerSelected ? (
        <form
