...
const AddSeedsOrderItemsDialog: React.FC<Props> = ({ onClose, orderId }) => {
  ...
  const calculateSum = () => {
    ...
    const total: string = orderItems
      .reduce((prev, next) => prev + next.kgPrice * next.quantity, 0)
      .toFixed(2)
    ...
    <Dialog open disableBackdropClick fullWidth onClose={onClose} aria-labelledby="dialog-title">
      ...
      <form
        onSubmit={handleSubmit(values => {
          return addSeedsOrderItems.mutate(values.seedsOrderItems, {
            onSuccess: () => {
              onClose()
            },
            onError: e => {
...
export default AddSeedsOrderItemsDialog
